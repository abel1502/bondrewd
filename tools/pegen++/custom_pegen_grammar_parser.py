...
# @generated by pegen from D:/Workspace/.MIPT/s6/bondrewd/tools/pegen++//custom_pegen_metagrammar.gram
...
class CustomGrammarParser(Parser):
    ...
    def rule(self) -> Optional[Rule]:
        # rule: rulename memoflag? ":" alts NEWLINE INDENT more_alts DEDENT | rulename memoflag? ":" NEWLINE INDENT more_alts DEDENT | rulename memoflag? ":" alts NEWLINE | rulename memoflag? ":" NEWLINE !INDENT
        mark = self._mark()
        if (
        ...
        if (
            ...
            return Rule ( rulename [0] , rulename [1] , Rhs ( [] ) , memo = opt );
        self._reset(mark)
        return None;
    ...
    def alt(self) -> Optional[Alt]:
        # alt: items '$' action | items '$' | items action | action | items
        mark = self._mark()
        if (
            (items := self.items())
            and
...
if __name__ == '__main__':
    ...
    simple_parser_main(CustomGrammarParser)
