...
def get_autogen_label(tool_name: str | None = None) -> str:
    if tool_name is None:
        tool_name = _get_tool_proj_path().as_posix()
...
def make_env(template_dir: pathlib.Path | str | None = None, **kwargs) -> jinja2.Environment:
    ...
    env.filters.update(dict(
        cpp_escape=lambda s: codecs.encode(s, "unicode_escape").decode("ascii") \
            .replace("'", "\\'").replace('"', '\\"'),
    ))
    
    env.tests.update(dict(
        instanceof=lambda obj, cls: isinstance(obj, cls),
    ))
