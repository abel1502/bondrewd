...
protected:
    ...
    #ifdef BONDREWD_PARSER_IMPL
    {
        ...
        { // 'ctime'? 'var' name type_annotation? ['=' expr] ';'
            ...
            auto _literal = lexer.expect().punct(lex::Punct::SEMI);
            if (_literal) {
                _res = ast::VarDef ( std::move ( n ) , t . value_or ( nullptr ) , v . value_or ( nullptr ) , true , bool ( c ) );
                return _res;
            }
            }
            }
    ...
    #ifdef BONDREWD_PARSER_IMPL
    {
        ...
        { // class_def
            auto _single_result = parse_class_def_rule();
            if (_single_result) {
                _res = std::move(_single_result);
                return _res;
            }
    ...
    #ifdef BONDREWD_PARSER_IMPL
    {
        ...
        { // 'ctime'? 'class' name '(' func_args_def ')' ';'
            ...
            if (_literal_2) {
                _res = ast::ClassDef ( std::move ( n ) , ast::Constant ( std::monostate ( ) ) , bool ( c ) );
                return _res;
            }
            }
            }
    ...
    #ifdef BONDREWD_PARSER_IMPL
    {
        ...
        { // stmt
            while (true) {
                ...
                if (_single_result) {
                    _res = std::move(_single_result);
                    _children.push_back(std::move(_res.value()));
                    _state = tell();
                    continue;
                }
                break;
        ...
        auto _seq = ast::make_sequence<ast::stmt>(std::move(_children));
        return _seq;
    }
    #else
    ;
    ...
    #ifdef BONDREWD_PARSER_IMPL
    {
        ...
        auto _seq = ast::make_sequence<ast::expr>(std::move(_children));
        return _seq;
    }
    #else
    ;
    ...
    #ifdef BONDREWD_PARSER_IMPL
    {
        auto _state = tell();
        std::optional<std::pair < ast::cmp_op , ast::expr >> _res = std::nullopt;
        std::vector<std::pair < ast::cmp_op , ast::expr >> _children{};
        size_t _n = 0;
        { // comparison_followup_pair
            while (true) {
